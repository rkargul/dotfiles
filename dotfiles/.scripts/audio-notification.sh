...
VOLUME=$(pactl list sinks | grep -oPze "(?s)Sink #$(get_active_sonk.sh sinks).*?Volume.*?\K(\d+)%" | head -c -2)
MUTED=$(pacmd list-sinks | grep -oPze "(?s)index: $(get_active_sonk.sh sinks).*?muted:\s+\K(\w+)")
